...
func RegisterRealWorldHTTPServer(s *http.Server, srv RealWorldHTTPServer) {
	...
	r.POST("/api/user", _RealWorld_Register0_HTTP_Handler(srv))
	...
	r.POST("/api/user/{username}/follow", _RealWorld_FollowUser0_HTTP_Handler(srv))
	r.DELETE("/api/user/{username}/follow", _RealWorld_UnFollowUser0_HTTP_Handler(srv))
...
func (c *RealWorldHTTPClientImpl) FollowUser(ctx context.Context, in *FollowUserRequest, opts ...http.CallOption) (*ProfileReplay, error) {
	...
	pattern := "/api/user/{username}/follow"
	path := binding.EncodeURL(pattern, in, false)
